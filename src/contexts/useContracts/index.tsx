...
export const useContracts = (context: ConnectedWeb3Context) => {
  ...
  const liftoffEngine = useMemo(
    () => new LiftoffEngineService(liftoffEngineAddress, provider, account),
  ...
  const liftoffInsurance = useMemo(
    () =>
      new LiftoffInsuranceService(liftoffInsuranceAddress, provider, account),
  ...
  const liftoffRegistration = useMemo(
    () =>
      new LiftoffRegistrationService(
